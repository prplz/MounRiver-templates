...
def OpenFile():
	global TextSrcFd,BinaryFd,TextDeviceFd,TextPropertyFd,BinOutFileStr,BackupWorkPath,HeaderFilePath
	...
	if len(InputCmdList) == (ArgvLen+1):
		HeaderFilePath = InputCmdList[2]
...
def GetVersionInfo():
	...
	if VersionIndex == -1:
		...
		return False
	VersionEndIndex = TextStr.find('\r',VersionIndex,len(TextStr))
	if VersionEndIndex == -1:
		VersionEndIndex = TextStr.find('\n',VersionIndex,len(TextStr))
	VersionStr = TextStr[VersionEndIndex-5:VersionEndIndex]
...
def PropertyNameParsing(InputStr,PropertyEncodeBinList):
	...
	for i in range(0,len(PropertyNamingSplitList),1):   # calculate the element size 
		if PropertyNamingSplitList[i] == 'u64':
			if IsString == 1:
				ElementSize = 0
				break
			ElementSize = ElementSize + 8
			ElementSizeList.append(8)
		elif PropertyNamingSplitList[i] == 'u32':
			if IsString == 1:
				ElementSize = 0
				break
...
def EndIdentifierParsing(InputStr):
	...
	for i in range (0,len(PropertyContentList),1):
		...
		if ElementSizeList[num] == 8:
			PackFormat = "Q"
		elif ElementSizeList[num] == 4:
			PackFormat = "I"
...
def SupportHeaderFile():
	global TextSrcFd,TotalDefinelist,HeaderFilePath
	...
	for i in range(0,len(SrcList),1):
		...
		if TmpStr.find('#include') >= 0:
			...
			if LeftPos == -1 or RightPos == -1:
				...
				#print (PathStr)
				if len(HeaderFilePath) == 0:
					PathList.append(PathStr)
				else:    # replace the header path from argc
					TempList = PathStr.split('/')
					FileName = TempList[len(TempList)-1]
					HeaderFileName = HeaderFilePath + FileName
					#print (HeaderFileName)
					PathList.append(HeaderFileName) 
