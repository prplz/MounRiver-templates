...
enumSysDescErrCode _Drv_SysDesc_FindPropertyID(unsigned short u16DeviceID, sysDescConfig *ops)
{
    ...
    ops->u16Device1stPropertyAddr_Bin = (u8SysDesTable[ops->u16DeviceAddr+1] << 8) | u8SysDesTable[ops->u16DeviceAddr];
...
enumSysDescErrCode _Drv_SysDesc_GetPropertyContent( unsigned short u16DeviceID,
                                                                    unsigned short u16PropertyID,
                                                                    unsigned char *pu8GetData,
                                                                    unsigned short *pu16GetData,
                                                                    unsigned int *pu32GetData,
                                                                    unsigned long long *pu64GetData,
                                                                    ...
                                                                    unsigned char u8DataType)
{
    ...
    enumSysDescErrCode enRet =E_SYS_DESC_PASS;
    unsigned short u16TrueLen = u16Length*u8DataType;
    unsigned char u8DataArray[QWORD_SIZE], i = 0;
    ...
    while(1)
    {
        if (current_ops.u16PropertyID_Bin == u16PropertyID)
        {
            ...
            current_ops.u16ContentLen_Bin = (u8SysDesTable[current_ops.u16ContentLenAddr+1] << 8) | u8SysDesTable[current_ops.u16ContentLenAddr];
            ...
            while(u16TrueLen--)
            {
                ...
                if (i == u8DataType)
                {
                    ...
                    else if (u8DataType == DWORD_SIZE)
                        *pu32GetData++ = (u8DataArray[3] << 24) | (u8DataArray[2] << 16) | (u8DataArray[1] << 8) | u8DataArray[0];
                    else
                        *pu64GetData++ = ((u64)u8DataArray[7] << 56) | ((u64)u8DataArray[6] << 48) | ((u64)u8DataArray[5] << 40) | ((u64)u8DataArray[4] << 32) |
                                         ((u64)u8DataArray[3] << 24) | ((u64)u8DataArray[2] << 16) | ((u64)u8DataArray[1] << 8) | ((u64)u8DataArray[0]);
                    i = 0;
                }
        ...
        else   // In the same of Device ID , to find next property ID
        {
            ...
            current_ops.u16PropertyID_Bin = (u8SysDesTable[current_ops.u16PropertyAddr+1] << 8) | u8SysDesTable[current_ops.u16PropertyAddr];
        }
    }
...
enumSysDescErrCode MDrv_SysDesc_GetElemsOfSize(unsigned short u16DeviceID, unsigned short u16PropertyID, unsigned short *pu16GetData)
{
    ...
    while(1)
    {
        if(current_ops.u16PropertyID_Bin == u16PropertyID)
        {
            ...
            current_ops.u16ElementSize_Bin = (u8SysDesTable[current_ops.u16ElementSizeAddr+1] << 8) | u8SysDesTable[current_ops.u16ElementSizeAddr];
...
{
    return _Drv_SysDesc_GetPropertyContent(u16DeviceID, u16PropertyID, pu8GetData, NULL, NULL, NULL, u16Length, BYTE_SIZE);
}
...
{
    return _Drv_SysDesc_GetPropertyContent(u16DeviceID, u16PropertyID, NULL, pu16GetData, NULL, NULL, u16Length, WORD_SIZE);
}
...
{
    return _Drv_SysDesc_GetPropertyContent(u16DeviceID, u16PropertyID, NULL, NULL, pu32GetData, NULL, u16Length, DWORD_SIZE);
}
...
{
    return _Drv_SysDesc_GetPropertyContent(u16DeviceID, u16PropertyID, NULL, NULL, NULL, pu64GetData, u16Length, QWORD_SIZE);
}
...
{
    return MDrv_SysDesc_Read_U64_Array(u16DeviceID, u16PropertyID, pu64GetData, 1);
}
...
{
    return _Drv_SysDesc_GetPropertyContent(u16DeviceID, u16PropertyID, (unsigned char *)pGetString, NULL, NULL, NULL, u16Length, BYTE_SIZE);
}
