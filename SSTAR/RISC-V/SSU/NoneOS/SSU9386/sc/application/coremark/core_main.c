...
ee_u32 benchmark_value = 0;
#if MAIN_HAS_NOARGC
MAIN_RETURN_TYPE coremark_main(CLI_t* pCli, char* p) {
	int argc=0;
	char *argv[1];
#else
...
#error "Please define a way to initialize a memory block."
	...
	if (results[0].iterations==0) {
		...
		while (secs_passed*10 < (secs_ret)1) {
			results[0].iterations+=100;
			start_time();
			iterate(&results[0]);
			stop_time();
			secs_passed=time_in_secs(get_time());
		}
		/* now we know it executes for at least 0.1 sec, set actual run time at about LEAST_SECS secs */
		divisor=(ee_u32)secs_passed;
		if (divisor==0) /* some machines cast float to int as 0 since this conversion is not defined by ANSI, but we know at least one second passed */
			divisor=1;
		results[0].iterations*=LEAST_SECS*10/divisor;
	}
...
#endif
	if (time_in_secs(total_time) < LEAST_SECS) {
		ee_printf("ERROR! Must execute for at least %d secs for a valid result!\n", LEAST_SECS);
		total_errors++;
	}
	...
	if (total_errors==0) {
		...
		benchmark_value = (int)(default_num_contexts*results[0].iterations/time_in_secs(total_time));
#if HAS_FLOAT
		if (known_id==3) {
...
SS_RTOS_CLI_CMD(coremark, "coremark test", "coremark test", coremark_main);
